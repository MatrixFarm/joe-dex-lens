...
import {ISafeAccessControlEnumerable, SafeAccessControlEnumerable} from "solrary/access/SafeAccessControlEnumerable.sol";
...
contract JoeDexLens is SafeAccessControlEnumerable, IJoeDexLens {
    ...
    constructor(ILBFactory lbFactory, ILBLegacyFactory lbLegacyFactory, IJoeFactory joeFactory, address wnative) {
        ...
        if (
            (address(lbFactory) == address(0) &&
                address(lbLegacyFactory) == address(0) &&
     ...
     */
    function getTokensPricesNative(address[] calldata tokens) external view override returns (uint256[] memory prices) {
    ...
    function _addDataFeed(
        address token,
        DataFeed calldata dataFeed
    ) private verifyDataFeed(token, dataFeed) verifyWeight(dataFeed.dfWeight) {
    ...
    function _addDataFeeds(
        address[] calldata tokens,
        DataFeed[] calldata dataFeeds
    ) private verifyLengths(tokens.length, dataFeeds.length) {
    ...
    function _removeDataFeeds(
        address[] calldata tokens,
        address[] calldata dfAddresses
    ) private verifyLengths(tokens.length, dfAddresses.length) {
        for (uint256 i; i < tokens.length; ) {
     ...
     */
    function _getDataFeedPrice(
        DataFeed memory dataFeed,
        address token
    ) private view returns (uint256 dfPrice, uint256 dfWeight) {
        DataFeedType dfType = dataFeed.dfType;
     ...
     */
    function _getTokenWeightedAverageNativePrices(
        address[] calldata tokens
    ) private view returns (uint256[] memory prices) {
        prices = new uint256[](tokens.length);
        for (uint256 i; i < tokens.length; ) {
     ...
     */
    function _getPriceFromV1(
        address pairAddress,
        address token
    ) private view returns (uint256 reserve0, uint256 reserve1, uint256 price, bool isTokenX) {
        ...
        if (isTokenX) {
            price = reserve0 == 0
                ? 0
                : (reserve1 * 10 ** (decimals0 + _WNATIVE_DECIMALS)) / (reserve0 * 10 ** decimals1);
        } else {
            price = reserve1 == 0
                ? 0
                : (reserve0 * 10 ** (decimals1 + _WNATIVE_DECIMALS)) / (reserve1 * 10 ** decimals0);
        }
    }
     ...
     * @return isTokenX True if the token is the first token of the pair, false otherwise
     */
    function _getPriceFromLb(
        address pair,
        DataFeedType dfType,
        address token
    ) private view returns (uint24 activeId, uint16 binStep, uint256 price, bool isTokenX) {
     ...
     */
    function _getPairedTokens(address pair, DataFeedType dfType) private view returns (address tokenA, address tokenB) {
     ...
     * @return binStep The bin step of the pair
     */
    function _getActiveIdAndBinStep(
        address pair,
        DataFeedType dfType
    ) private view returns (uint24 activeId, uint16 binStep) {
     ...
     */
    function _v2_1FallbackNativePrice(address token) private view returns (uint256 weightedPrice, uint256 totalWeight) {
        if (address(_FACTORY_V2_1) == address(0)) return (0, 0);

        ILBFactory.LBPairInformation[] memory lbPairsAvailable = _FACTORY_V2_1.getAllLBPairs(
            IERC20(_WNATIVE),
            IERC20(token)
        );
        ...
        if (lbPairsAvailable.length != 0) {
            for (uint256 i = 0; i < lbPairsAvailable.length; i++) {
                ...
                (uint24 activeId, uint16 binStep, uint256 price, bool isTokenX) = _getPriceFromLb(
                    lbPair,
                    DataFeedType.V2_1,
                    token
                );
    ...
    function _v2FallbackNativePrice(address token) private view returns (uint256 weightedPrice, uint256 totalWeight) {
        ...
        ILBLegacyFactory.LBPairInformation[] memory lbPairsAvailable = _LEGACY_FACTORY_V2.getAllLBPairs(
            IERC20(_WNATIVE),
            IERC20(token)
        );
     ...
     */
    function _getLbBinReserves(
        address lbPair,
        uint24 activeId,
        uint16 binStep,
        bool isTokenX
    ) private view returns (uint256 scaledReserves) {
        if (isTokenX) {
